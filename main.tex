...
\date{\today}
\renewcommand{\zjuthanksinfo}{张三 · 浙江大学计算机科学与技术学院 · \today}
  ...
  \begin{minipage}[t]{0.475\textwidth}
    ...
    \begin{zjubox}{blue}{环境依赖}
      ...
      \begin{denseitem}
        ...
        \item 编译器：\textbf{XeLaTeX}，编译两遍
        \item 字体已随 \texttt{fonts/} 分发，无需安装
      \end{denseitem}}
    \end{zjubox}
    \begin{zjubox}{orange}{在哪里改内容}
      {\footnotesize 所有正文都在 \texttt{main.tex}；
      封面信息在开头的 \texttt{\string\title} 等命令中。}
    \end{zjubox}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.475\textwidth}
    \vspace{0pt}%
    \begin{zjubox}{green}{Overleaf}
      {\footnotesize 上传 Release 中的模板压缩包
      $\rightarrow$ 编译器切换为 \textbf{XeLaTeX}
      $\rightarrow$ 编译两次。}
    \end{zjubox}
    \begin{zjubox}{gray}{自动生成}
      {\footnotesize 封面、章节过渡页、致谢页均由模板
      自动生成，无需手写。}
    \end{zjubox}
  \end{minipage}
  \vspace{0.8mm}
